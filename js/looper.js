...
(() => {
  ...
  let performanceMode = false, perfTrackIndex = -1, perfAutoStopTimer = null;
  let perfModeBtn = null;
  ...
  perfModeBtn = document.getElementById('perfModeBtn');
  if (perfModeBtn) {
    perfModeBtn.addEventListener('click', () => {
      if (performanceMode) { stopPerformanceMode(); } else { startPerformanceMode(); }
    });
  }
  ...
  // ====== COUNT-IN ======
  // Schedules N measures of metronome clicks, sets startAt, then calls onComplete
  function doCountIn(measures, onComplete) {
    if (!audioCtx) return;
    if (!measureLength) measureLength = computeMeasureLength();
    const countInDuration = measures * measureLength;
    const kickAt = audioCtx.currentTime + 0.05;

    // Schedule all beats for the count-in window only
    const totalBeats = Math.round(measures * tsTop);
    const beatDur = measureLength / tsTop;
    for (let b = 0; b < totalBeats; b++) {
      const isAccent = (b % tsTop === 0);
      scheduleMetroTickAt(kickAt + b * beatDur, isAccent);
    }

    // startAt anchors the loop grid from the moment recording begins
    startAt = kickAt + countInDuration;

    // playing must be true NOW so _beginRecording sets recSchedule and
    // applyBufferToTrack starts each source node after recording completes
    playing = true;
    requestAnimationFrame(tickProgress);
    ...
    countInTimer = setTimeout(() => {
      countInActive = false;
      countInTimer = null;
      stopMetronome(); // silence metronome so it won't bleed into recording
      onComplete();
    }, Math.round(countInDuration * 1000) + 80);
  }
  ...
  function startPerformanceMode() {
    ...
    perfTrackIndex = -1;
    if (perfModeBtn) {
      perfModeBtn.textContent = '⏹ Stop Perf';
      perfModeBtn.style.cssText = 'background:#3a1717;border-color:#ff5252;color:#ff8a8a';
    }
    toast('Perf mode ON — press Rec or tap T1 to begin');
  }

  function stopPerformanceMode() {
    performanceMode = false;
    perfTrackIndex = -1;
    if (perfAutoStopTimer) { clearTimeout(perfAutoStopTimer); perfAutoStopTimer = null; }
    if (countInTimer) { clearTimeout(countInTimer); countInTimer = null; countInActive = false; }
    if (perfModeBtn) {
      perfModeBtn.textContent = '🎭 Performance';
      perfModeBtn.className = 'btn';
      perfModeBtn.style.cssText = 'background:#1a1a2e;border-color:#7c3aed;color:#a78bfa';
    }
  }

  function advancePerfMode(justFinishedIndex) {
    const nextIndex = justFinishedIndex + 1;
    if (nextIndex >= N || nextIndex >= tracks.length) {
      stopPerformanceMode();
      toast('All tracks recorded — looping');
      return;
    }
    const t = tracks[nextIndex];
    t.stopBtn.disabled = false;
    t.stopBtn.textContent = 'Stop';
    t.stopBtn.className = 'btn small red';
    toast('Perf: recording T' + (nextIndex + 1));
    _beginRecording(nextIndex, t.recBtn, t.stopBtn);
  }
  ...
  async function safeRecordStart(i, recBtn, toggleBtn) {
    if (recBusy || countInActive) return;
    recBusy = true;
    tr(i).recChip.textContent = 'Arm';
    await ensureAudioReady();
    const ok = await ensureMic();
    if (!ok) { tr(i).recChip.textContent = 'Err'; recBusy = false; restoreMonitorAfterRecord(); return; }
    ...
    if (isFirstRec) {
      ...
      tr(i).recChip.textContent = 'IN';
      if (tr(i).easyStatusEl) tr(i).easyStatusEl.textContent = 'COUNT-IN';
      if (tr(i).labelEl) tr(i).labelEl.classList.add('recording-active');
      if (tr(i).easyPadEl) tr(i).easyPadEl.classList.add('recording');

      doCountIn(1, () => {
        _beginRecording(i, recBtn, toggleBtn);
      });
    } else {
      _beginRecording(i, recBtn, toggleBtn);
    }
  ...
  function _beginRecording(i, recBtn, toggleBtn) {
    recordingTrack = i; chunks = [];
    tr(i).statusEl.textContent = 'Rec…';
    tr(i).progressEl.classList.add('rec');
    tr(i).recChip.className = 'chip rec'; tr(i).recChip.textContent = 'REC';
    if (tr(i).labelEl) tr(i).labelEl.classList.add('recording-active');
    if (tr(i).easyPadEl) tr(i).easyPadEl.classList.add('recording');
    if (tr(i).easyStatusEl) tr(i).easyStatusEl.textContent = 'RECORDING';

    const usingMediaRecorder = !!mediaRec;
    if (usingMediaRecorder && mediaRec.state !== 'inactive') {
      try { mediaRec.stop(); } catch(e) {}
    }
    if (loopLenSec && playing) {
      const now = audioCtx.currentTime;
      const currentLoopOffset = (now - startAt) % loopLenSec;
      if (usingMediaRecorder) { try { mediaRec.start(); } catch(e) { endRecUI(i); return; } } else { fallbackRec.start(); }
    ...
    // Performance mode: auto-stop at the next loop boundary
    if (performanceMode && loopLenSec) {
      if (perfAutoStopTimer) clearTimeout(perfAutoStopTimer);
      const now = audioCtx.currentTime;
      const elapsed = playing ? (now - startAt) % loopLenSec : 0;
      const timeToLoopEnd = loopLenSec - elapsed;
      perfAutoStopTimer = setTimeout(() => {
        if (recordingTrack === i) stopRecording();
      }, Math.round(timeToLoopEnd * 1000) - 60);
    }
  }
  ...
  function processRecordedBuffer(i, buf){
    ...
    requestAnimationFrame(tickProgress);
    // Performance mode: advance to next track
    if (performanceMode) advancePerfMode(i);
  }
