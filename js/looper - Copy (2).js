/* FILE: js/looper.js
   UPDATED: Merged "Punch-In" Logic with History/Undo Features
*/
(() => {
  // ====== Tunables ======
  ...
  function enableControls(on){
    document.querySelectorAll('.ctrls .btn').forEach(b=>{
      ...
      else if (label==='Stop Rec' || label==='Undo' || label==='Redo') b.disabled = true;
  ...
  // ====== Metronome / Tempo / Time Signature / Swing Logic ======
  function computeMeasureLength(){
    const beatSec = 60 / bpm; 
    const scale = 4 / tsBottom; 
  ...
  function metronomeScheduler(){
    ...
    while (metroNextSubdivTime < now + scheduleAheadTime){
      ...
      const beatIndex = Math.floor(idx / metroPulseSubdivisions); 
      const subdivIndexInBeat = idx % metroPulseSubdivisions; 
      const isAccent = (beatIndex === 0 && subdivIndexInBeat === 0);
      let pulseDuration = beatDuration / metroPulseSubdivisions; 
      
      if (metroPulseSubdivisions === 2 && swingPercent > 0) {
        const firstDur = beatDuration * (0.5 + (swingPercent/100)/2); 
        const secondDur = beatDuration - firstDur; 
        if (subdivIndexInBeat === 0){ pulseDuration = firstDur; } else { pulseDuration = secondDur; }
      }
  ...
  function setupTempoUI(){
    ...
    metroPulseSubdivisions = Number(metroSubdivisionsSel.value) || 1; 
    updateMeasureDisplay();

    bpmInput.addEventListener('change', ()=>{
      bpm = Math.max(40, Math.min(240, Number(bpmInput.value) || 120));
      bpmInput.value = bpm; bpmSlider.value = bpm; updateMeasureDisplay();
      if (metronomeEnabled){ stopMetronome(); startMetronome(); }
    });
    bpmSlider.addEventListener('input', ()=>{
      bpm = Number(bpmSlider.value); bpmInput.value = bpm; updateMeasureDisplay();
      if (metronomeEnabled){ stopMetronome(); startMetronome(); }
    });
    timeTopSel.addEventListener('change', ()=>{
      tsTop = Number(timeTopSel.value) || 4; updateMeasureDisplay();
      if (metronomeEnabled){ stopMetronome(); startMetronome(); }
    });
    timeBottomSel.addEventListener('change', ()=>{
      tsBottom = Number(timeBottomSel.value) || 4; updateMeasureDisplay();
      if (metronomeEnabled){ stopMetronome(); startMetronome(); }
    });
    swingSlider.addEventListener('input', ()=>{
      swingPercent = Number(swingSlider.value); swingLabel.textContent = swingPercent + '%';
    });
  ...
  function pushHistory(i) {
    ...
    if (t.historyPointer < t.history.length - 1) { t.history.splice(t.historyPointer + 1); }
  ...
  // ====== Recording (UPDATED FOR PUNCH-IN) ======
  async function safeRecordStart(i,recBtn,stopRecBtn){
    ...
    const ok=await ensureMic();
    if(!ok){
      tr(i).recChip.className='chip idle';
      tr(i).recChip.textContent='Idle';
      recBusy=false;
      restoreMonitorAfterRecord();
      return;
    }
    monitorPrevValue=monitorGain?monitorGain.gain.value:0;
    if(monitorGain)monitorGain.gain.value=0.0;
    ...
    tr(i).recChip.className='chip rec';
    tr(i).recChip.textContent='REC';
    ...
    if(loopLenSec && playing){
        // IMMEDIATE PUNCH-IN
        const now = audioCtx.currentTime;
        const currentLoopOffset = (now - startAt) % loopLenSec;
        
        if(usingMediaRecorder){
            try{ mediaRec.start(); }catch(e){
                toast('Recorder failed.'); endRecUI(i); recBusy=false; restoreMonitorAfterRecord(); return;
            }
        } else {
            fallbackRec.start();
        }
        
        recSchedule = { track: i, offset: currentLoopOffset };
        sweepRecordLEDs(i);
    } else {
        // First track / free recording
        if(usingMediaRecorder){
            try{ mediaRec.start(); }catch(e){
                toast('Recorder failed.'); endRecUI(i); recBusy=false; restoreMonitorAfterRecord(); return;
            }
  ...
  function stopRecording(){
    ...
    const when=t+NO_LOOP_STOP_GRACE_MS/1000;
    if(mediaRec&&mediaRec.state==='recording'){
      waitUntil(when,()=>{try{if(mediaRec.requestData)mediaRec.requestData()}catch{}mediaRec.stop()})
    } else if(fallbackRec&&fallbackRec.isRecording){
      waitUntil(when,()=>{fallbackRec.stop();onRecordingComplete_Fallback(recordingTrack)})
    }
  }

  function waitUntil(whenCtxTime,fn){
    const tick=()=>{if(!audioCtx)return;if(audioCtx.currentTime>=whenCtxTime-0.001){fn();return}requestAnimationFrame(tick)};
    requestAnimationFrame(tick)
  }

  // ====== Rec Complete Handlers (UPDATED FOR OFFSET PADDING) ======
  async function onRecordingComplete_MediaRecorder(){
    const i=recordingTrack;
    recordingTrack=-1;
    tr(i).recChip.className='chip idle';
    tr(i).recChip.textContent='Idle';
    ...
    let ab; try{ ab=await blob.arrayBuffer(); }catch{ endRecUI(i); restoreMonitorAfterRecord(); return; }
    let buf=await audioCtx.decodeAudioData(ab).catch(()=>null);
    if(!buf){ toast('Decode failed.'); endRecUI(i); restoreMonitorAfterRecord(); return; }
    buf=toMonoWithFades(buf,0.004);

    if(recSchedule && recSchedule.track === i && recSchedule.offset !== undefined){
        const sr = buf.sampleRate;
        const totalSamples = Math.floor(loopLenSec * sr);
        const offsetSamples = Math.floor(recSchedule.offset * sr);
        const out = audioCtx.createBuffer(1, totalSamples, sr);
        const outData = out.getChannelData(0);
        const inData = buf.getChannelData(0);
        const availableSpace = totalSamples - offsetSamples;
        const copyLen = Math.min(inData.length, availableSpace);
        
        outData.set(inData.subarray(0, copyLen), offsetSamples);
        if (inData.length > availableSpace) {
            const remaining = inData.subarray(availableSpace);
            const wrapLen = Math.min(remaining.length, offsetSamples); 
            outData.set(remaining.subarray(0, wrapLen), 0);
        }
        buf = out;
        recSchedule = null;
    } else {
  ...
  applyLoopBtn.addEventListener('click',async()=>{
    ...
    tracks.forEach((t,idx)=>{ 
      if (!conformed[idx] && t.buffer) { applyBufferToTrack(idx, conformed[idx]); } 
      else if (conformed[idx]) { applyBufferToTrack(idx, conformed[idx]); }
      pushHistory(idx); 
    });
    loopLenSec = newLen; 
    setLoopLenDisplay();
  ...
  function setLoopLenDisplay(){ 
    if(!loopLenSec){ loopBadge.style.display='none'; loopMeasuresInput.value = '4'; return; } 
  ...
  loadSessBtn.addEventListener('click',async()=>{if(!hiddenFileInput){hiddenFileInput=document.createElement('input');hiddenFileInput.type='file';hiddenFileInput.accept='application/json';hiddenFileInput.style.display='none';document.body.appendChild(hiddenFileInput);hiddenFileInput.addEventListener('change',async(e)=>{const file=e.target.files[0];if(!file)return;const text=await file.text().catch(()=>null);if(!text){toast('Load failed.');return}let payload=null;try{payload=JSON.parse(text)}catch{toast('Invalid session file.');return}await ensureAudioReady();stopPlayback();
            ...
            loopLenSec = measures * measureLength;
            setLoopLenDisplay(); 
            const rowEls=Array.from(document.querySelectorAll('.tracks .track'));for(let i=0;i<tracks.length;i++){const t=tr(i);const data=payload.tracks?.[i];if(t.source){try{t.source.stop()}catch{}t.source.disconnect();t.source=null}t.buffer=null;t.history=[];t.historyPointer=-1; 
            ...
            if(data?.wavDataUrl){
              ...
              if(buf){ applyBufferToTrack(i, normalizeToLoop(toMonoWithFades(buf,0.004),loopLenSec)); }
